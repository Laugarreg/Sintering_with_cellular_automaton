...
REAL(8) :: rnd				! Auxiliar para circulo inicial [0,1)
REAL(8) :: rnd_circ			! Auxiliar para circulo aleatorio [0,1)
REAL(8) :: rnd_phi			! Auxiliar para angulo aleatorio [0,1)
INTEGER :: contacto_duro	! Circulo elegido para contacto duro
...
PROGRAM CIRCULOS_CONTACTO_DURO
USE variables
...
OPEN(UNIT=11, FILE='circulos-contacto-duro.in', STATUS='OLD')
	READ(11,*) n_mat
	READ(11,*) rho
	READ(11,*) radio
	READ(11,*) n_int
...
DO WHILE (densidad_real.LT.rho .AND. intentos.LT.n_int)
	...
	! Se elige un circulo ya colocado para el contacto duro
	CALL RANDOM_NUMBER(rnd_circ)
	contacto_duro = 1 + NINT((circ_colocados-1)*rnd_circ)
	
	! Se elige un angulo para el contacto duro
	CALL RANDOM_NUMBER(rnd_phi)
	rnd_phi = 2.0d0*pi*rnd_phi	
	
	! Posicion para el centro del nuevo circulo
	x_new = circulos(contacto_duro,1) + 2.0d0*radio*COS(rnd_phi)
	y_new = circulos(contacto_duro,2) + 2.0d0*radio*SIN(rnd_phi)
	
	! Se comprueba que no solape con ninguno colocado previamente
	DO i=1,circ_colocados
	
		! Distancias relativa
		x_rel = circulos(i,1) - x_new
		y_rel = circulos(i,2) - y_new
		
		! Correccion PBC
		x_rel = x_rel - DBLE(n_mat)*DNINT(x_rel/DBLE(n_mat)) 
		y_rel = y_rel - DBLE(n_mat)*DNINT(y_rel/DBLE(n_mat)) 
		
		! Distancia
		dist = SQRT(x_rel**2.0d0 + y_rel**2.0d0)
		IF (dist.LT.(2.0d0*radio)) THEN	! Solapa
			intentos = intentos + 1
			GO TO 101
		ELSE		! No solapa
			CONTINUE
	...
	! Si no solapa, se añade el circulo, se actualiza la matriz y se 
	! recalcula la densidad_real
	intentos = 0
	circ_colocados = circ_colocados + 1
	circulos(circ_colocados,1) = x_new
	circulos(circ_colocados,2) = y_new
	...
	! Si solapa, salta aqui y realiza otro intento para colocar el 
	! siguiente circulo
	101 CONTINUE
ENDDO
...
END PROGRAM CIRCULOS_CONTACTO_DURO
